...
void Shutdown()
{
    ...
    RenameThread("MontBlanc-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: MontBlanc.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: MontBlancd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeMontBlancamount=<n> " + _("Keep N MontBlanc anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MontBlanc is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MontBlanc is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("MontBlanc version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "MontBlanc server starting\n"); 
...
#ifdef ENABLE_WALLET
    if (mapArgs.count("-reservebalance")) // MontBlancCoin: reserve balance amount
    {
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MontBlanc") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart MontBlanc to complete") << "\n";
    ...
    nAnonymizeMontBlancAmount = GetArg("-anonymizeMontBlancamount", 0);
    if(nAnonymizeMontBlancAmount > 999999) nAnonymizeMontBlancAmount = 999999;
    if(nAnonymizeMontBlancAmount < 2) nAnonymizeMontBlancAmount = 2;
    ...
    LogPrintf("Anonymize MontBlanc Amount %d\n", nAnonymizeMontBlancAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1MBLC+1000 == (.1MBLC+100)*10
       10MBLC+10000 == (1MBLC+1000)*10
    */
