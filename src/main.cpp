...
const string strMessageMagic = "MontBlanc Signed Message:\n";
                        ...
                        bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    // MontBlancCoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// MontBlancCoin: find block wanted by given orphan block
...
int64_t GetPosRewardTieredMN(const CBlockIndex* pindexPrev)
{
    ...
    if (nHeight > 50000 && nHeight <= 100000) 
...
// MontBlancCoin: find last block index up to pindex
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // MontBlancCoin: target change every block
    // MontBlancCoin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags, bool fValidateSig)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // MontBlancCoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // MontBlancCoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // MontBlancCoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // MontBlancCoin: track money supply and mint amount info
#ifndef LOWMEM
...
// MontBlancCoin: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // MontBlancCoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // MontBlancCoin: compute stake entropy bit for stake modifier
    ...
    // MontBlancCoin: compute stake modifier
                    ...
                    if(!checkoff){
                        ...
                        if(!winningNode){
                            ...
                            if(fDebug) {
                                ...
                                CMontBlancCoinAddress address23(addressnned1);
                                CMontBlancCoinAddress address24(addressnned2);
                            ...
                            for (unsigned int i = 0; i < vtx[1].vout.size(); i++) {
                                ...
                                if(fDebug) { CTxDestination addressfound;
                                    ...
                                    CMontBlancCoinAddress address25(addressfound);
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // MontBlancCoin: check proof-of-stake
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // MontBlancCoin: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// MontBlancCoin: attempt to generate suitable proof-of-stake
...
{
    RenameThread("MontBlanc-loadblk");
