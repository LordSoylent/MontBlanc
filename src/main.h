...
static const int64_t STATIC_POS_REWARD = 12.5 * COIN; //Constant reward of 12.5 MBLC per block
static const int64_t POS_REWARD_TIERED_MN_LVL1 = 3.2* COIN; // from 0 block
static const int64_t POS_REWARD_TIERED_MN_LVL2 = 2.5 * COIN; // from 50000  block
static const int64_t POS_REWARD_TIERED_MN_LVL3 = 2 * COIN;  // from 100000  block
...
static const int64_t MAX_MONEY = 70000000 * COIN;  // 70000000 COINS
...
public:
    ...
    {
        // MontBlancCoin: the coin stake transaction is marked with the first output empty
...
public:
    ...
    // MontBlancCoin: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // MontBlancCoin: two types of block: proof-of-work or proof-of-stake
    ...
    // MontBlancCoin: get max transaction timestamp
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // MontBlancCoin: trust score of block chain
    int nHeight;
#ifndef LOWMEM
    int64_t nMint;
    int64_t nMoneySupply;
#endif
    unsigned int nFlags;  // MontBlancCoin: block index flags
    enum
    {
