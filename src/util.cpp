...
int nAnonymizeMontBlancAmount = 1000;
...
#else
    const char* pszModule = "MontBlanc";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MontBlanc
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MontBlanc
    // Mac: ~/Library/Application Support/MontBlanc
    // Unix: ~/.MontBlanc
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MontBlanc";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "MontBlanc.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "MontBlancd.pid"));
