...
{
    // MontBlancCoin: check prefix
    if(uri.scheme() != QString("MontBlanc"))
        return false;
...
{
    // Convert MontBlanc:// to MontBlanc:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("MontBlanc://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "MontBlanc:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MontBlanc.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for MontBlanc.lnk
...
{
    return GetAutostartDir() / "MontBlanc.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a MontBlanc.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=MontBlanc\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the MontBlanc app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add MontBlanc app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("MontBlanc-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  MontBlanc-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("MontBlanc-Qt"));
