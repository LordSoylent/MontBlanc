...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeMontBlancAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeMontBlancAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeMontBlancAmount = strAnonymizeMontBlancAmount.remove(strAnonymizeMontBlancAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMontBlancAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeMontBlancAmount*COIN) nMaxToAnonymize = nAnonymizeMontBlancAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeMontBlancAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeMontBlancAmount));
        strAnonymizeMontBlancAmount = strAnonymizeMontBlancAmount.remove(strAnonymizeMontBlancAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMontBlancAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeMontBlancAmount == 0){
