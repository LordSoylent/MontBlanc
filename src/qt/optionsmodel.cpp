...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeMontBlancAmount"))
        settings.setValue("nAnonymizeMontBlancAmount", 1000);
    nAnonymizeMontBlancAmount = settings.value("nAnonymizeMontBlancAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeMontBlancAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeMontBlancAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeMontBlancAmount:
            return QVariant(nAnonymizeMontBlancAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeMontBlancAmount:
            nAnonymizeMontBlancAmount = value.toInt();
            settings.setValue("nAnonymizeMontBlancAmount", nAnonymizeMontBlancAmount);
            emit AnonymizeMontBlancAmountChanged(nAnonymizeMontBlancAmount);
            break;
        default:
            break;
        }
